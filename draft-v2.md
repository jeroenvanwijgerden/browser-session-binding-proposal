...
**Post-Claim Code.** A short code displayed by the user agent after claiming and entered by the user into the companion application. See Section 7 for detailed analysis of its purpose and requirements.
...
dictionary BindingRequest {
  ...
  // Post-negotiate code format (one of these must be provided)
...
7. **Post-claim code area**: After claiming, displays the code with instructions to enter it in the companion application.
...
| NEGOTIATED | App(s) completed operation; approve_token(s) and post_negotiate_code(s) issued; awaiting browser claim |
| CLAIMED | Browser(s) claimed with valid post-negotiate code; complete_token(s) and post_claim_code(s) issued |
...
3. Displays trusted UI with:
   ...
   - **Post-negotiate code input field** (awaiting user entry)
...
5. Upon completion, receives from the service:
   - An approve token
   - A **post-negotiate code** (to be entered into the browser)
   - Post-claim code specification (character set, length)--see Section 7
...
2. Generates a post-negotiate code according to its chosen specification
3. Stores: `{session_id, result_data, approve_token, post_negotiate_code, expires_at}`
4. Returns the approve token, post-negotiate code, and post-claim code specification to the companion application
...
The companion application displays the post-negotiate code for the user to enter into the browser, and prepares to receive the post-claim code (e.g., displaying the appropriate number of input fields for the specified character set).

### 5.5 Phase 3: Claiming

The user reads the post-negotiate code from the companion application and enters it into the browser's trusted UI. The browser then sends a claim request to the claim endpoint with:
...
When the post-negotiate code is valid:

1. Generates a post-claim code according to its chosen specification
...
4. Returns the complete token and post-claim code to the user agent

The user agent displays the post-claim code and instructs the user to enter it in the companion application.

**Critical:** The service MUST allow multiple claims against the same session ID (provided each has a valid post-negotiate code). Each claim generates a separate complete token and post-claim code. This is essential for attack detection.

### 5.6 Phase 4: Approval

The user enters the post-claim code into the companion application. The application sends to the approve endpoint:
...
4. The post-claim code matches the claim
...
The post-negotiate code is a critical security feature that prevents session fixation attacks. This section explains the attack it prevents, why it's necessary, and how it works.
...
The post-claim code is a critical usability and security feature. This section explains its purpose, why its requirements differ from the session ID, and how services can configure it.
...
| claim | Browser | Register interest in result, receive complete_token + post_claim_code |
| approve | App | Submit post-claim code, trigger multi-claim check |
...
This protocol requires two user-entered codes: the post-negotiate code (app → browser) and the post-claim code (browser → app). This is one more code entry than some existing cross-device flows.
...
| This protocol | Enter 1 code (app → browser), then enter 1 code (browser → app), then approve in app |

**Why the difference:**

DigiD's companion app authenticates the user (via PIN or biometric) before communicating with the server. An attacker who scans the QR code cannot negotiate as the user—they lack the user's credentials. The single code (browser → app) binds the browser session to the authenticated negotiation.

This protocol makes no assumption about app-side authentication. The negotiation phase may not involve user credentials at all—for example, when binding an anonymous session or a pre-generated token. Because anyone can scan the QR and negotiate their own session, the post-negotiate code is necessary to ensure the user's browser claims the result of the user's app's negotiation, not an attacker's.

The additional code entry is the cost of supporting use cases where the app does not authenticate the user before negotiating.
...
|----------|----------|------|---------|
| initiate | session_id, operation data | Validate operation, store result + approve_token + post_negotiate_code | approve_token, post_negotiate_code, code_spec |
| claim | session_id, post_negotiate_code | Validate post_negotiate_code matches, record claim with new complete_token + post_claim_code | complete_token, post_claim_code |
...
The `post_negotiate_code` is displayed to the user by the companion app. The user enters it into the browser's trusted UI before the browser claims.
...
Called by the browser to register interest in receiving the result. The browser includes the post-negotiate code entered by the user.
...
**Response (ready - valid post-negotiate code):**
```json
HTTP/1.1 200 OK
Content-Type: application/json
...
Called by the companion application with the post-claim code.
