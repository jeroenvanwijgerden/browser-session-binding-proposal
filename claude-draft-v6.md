...
Different services have different security requirements. A bank authorizing a large transfer should require more friction than a forum login. This proposal recommends that services control the confirmation code specification.
...
**Length.** How many characters in the code. Trade-off between security and usability:

| Length | Digits (10) | Letters (26) | Alphanumeric (36) |
|--------|-------------|--------------|-------------------|
| 1 | 10 combinations | 26 | 36 |
| 2 | 100 | 676 | 1,296 |
| 3 | 1,000 | 17,576 | 46,656 |
| 4 | 10,000 | 456,976 | 1,679,616 |

**Recommendation:** At minimum, 2 decimal digits (1% random match probability). For high-value operations, 4+ characters or a larger character set.
...
The confirmation code is **not** the primary security mechanism. Multi-claim detection provides the cryptographic guarantee: if an attacker races to claim, the service detects multiple claims at approval time and aborts.

The confirmation code is a **defense in depth** that:
1. Reduces the chance of user error (approving before seeing the code)
2. Makes accidental collision with an attacker's code improbable
3. Provides psychological assurance that the browser and app are synchronized

Services should choose code parameters based on their risk tolerance. Very short codes (1 digit) provide minimal protection but maximum convenience. Longer codes provide more protection at the cost of friction.
...
| Confirmation code | Service-configured | Character set and length per service requirements |
