...
**Problem.** Security breaches remain rampant despite years of effort. One contributing factor: truly secure cross-device authentication is artificially scarce. Today, only two types of organizations can offer it:
...
Most services cannot afford this. Building a secure companion app requires specialized cryptographic expertise, app store presence, continuous security maintenance, and user support. The cost is prohibitive for all but the largest organizations.

**The result for users is fragmentation and insecurity.** Those who use well-resourced services get secure authentication—but must download, store, learn, and maintain separate apps for each service. Those who use smaller services get weaker security through no fault of their own. The accessible alternatives (manual code copy-paste, polling without binding) are insecure.

**Solution.** This protocol decouples secure cross-device binding from dedicated apps. Services implement four server endpoints instead of building companion applications. Users use *one* general-purpose companion app—a password manager, authenticator, or similar—for *all* services that implement the protocol.

The flow: A service's web page requests a browser-native, sandboxed UI displaying a QR code. The user's companion app scans it and negotiates with the service's server; the app receives a negotiation code. The user enters this code into the browser, which then claims the session. After claiming, the browser shows a confirmation code. The user enters this code into the companion app, which approves the binding. The browser retrieves the result.

**What this changes:**
- Services no longer need to build, distribute, or maintain dedicated apps—just server endpoints
- Users no longer need a different app per service—one companion app works everywhere
- Secure cross-device authentication becomes accessible to any service, not just well-resourced ones

**Key mechanism.** The protocol uses symmetric two-phase verification with two codes:

- **Negotiation code** (app → browser): After the companion app negotiates, it receives a code. The user enters this into the browser before claiming. If multiple apps negotiate for the same session, multi-negotiation detection aborts. This prevents session fixation attacks where an attacker observes the QR code and negotiates as themselves.

- **Confirmation code** (browser → app): After the browser claims, it displays a code. The user enters this into the app before approval. If multiple browsers claim the same session, multi-claim detection aborts. This prevents session hijacking attacks where an attacker observes the QR code and claims as themselves.

Both codes require user action that an observation-only attacker cannot perform. Both detection mechanisms reveal attacks. Together, they ensure only the legitimate user's browser receives the negotiated result.

**Properties:**
- **Phishing-resistant.** Browser enforces same-origin; attackers can only bind to their own origin.
- **Detects and neutralizes over-the-shoulder attacks** An attack made possible by vision of the user's browser will only cause the flow to be voided, never hijacked. In such an event, the user can be warned of a compromised environment.
...
- **Simple.** Four HTTPS endpoints, one expiring session, three tokens, and two user-entered codes.
...
dictionary BindingRequest {
  ...
  // Negotiation code format (one of these must be provided)
  CodeSpec negotiationCodeSpec;             // Format specified directly
  USVString negotiationCodeSpecEndpoint;    // OR: endpoint to fetch format from
...
```javascript
// Login example - with inline negotiation code spec
const result = await navigator.outOfBandBinding.request({
  ...
  negotiationCodeSpec: { charset: '0-9', length: 4 },
...
```javascript
// Payment example - with server-fetched negotiation code spec
const result = await navigator.outOfBandBinding.request({
  initiateEndpoint: '/bind/initiate',
  claimEndpoint: '/bind/claim',
  approveEndpoint: '/bind/approve',
  ...
  negotiationCodeSpecEndpoint: '/bind/negotiation-spec',  // Server controls format
...
└────┬─────┘   └─────┬──────┘   └──────┬──────┘   └────┬────┘
     ...
     │               │                 │<──────────────┤
     │               │                 │               │
     │               │ ─ App shows negotiation code ─ ─│
     │               │                 │               │
     │               │   User enters negotiation code  │
     │               │                 │               │
     │               │   Claim (with neg_code)         │
     │               ├────────────────────────────────>│
     │               │     complete_token + conf_code  │
     │               │<────────────────────────────────┤
     │               │                 │               │
...
│  EMPTY  │──────────────►│ NEGOTIATED │───neg_code)────►│ CLAIMED │◄─┐
└─────────┘   (success)   └─────┬──────┘                 └────┬────┘  │
                                │                              │       │
...
| NEGOTIATED | App(s) completed operation; approve_token(s) and negotiation_code(s) issued; awaiting browser claim |
| CLAIMED | Browser(s) claimed with valid negotiation code; complete_token(s) and confirmation_code(s) issued |
...
3. Displays trusted UI with:
   ...
   - **Negotiation code input field** (awaiting user entry)
...
5. Upon completion, receives from the service:
   - An approve token
   - A **negotiation code** (to be entered into the browser)
   - Confirmation code specification (character set, length)--see Section 7
...
2. Generates a negotiation code according to its chosen specification
3. Stores: `{session_id, result_data, approve_token, negotiation_code, expires_at}`
4. Returns the approve token, negotiation code, and confirmation code specification to the companion application

**Critical:** The service MUST allow multiple negotiations against the same session ID. Each negotiation generates a separate approve token and negotiation code. This is essential for multi-negotiation detection (see Section 6).
...
The companion application displays the negotiation code for the user to enter into the browser, and prepares to receive the confirmation code (e.g., displaying the appropriate number of input fields for the specified character set).

### 5.5 Phase 3: Claiming

The user reads the negotiation code from the companion application and enters it into the browser's trusted UI. The browser then sends a claim request to the claim endpoint with:

- The session ID
- The negotiation code

The service validates the negotiation code against stored negotiations:

1. If no negotiation exists for this session → respond with "pending"
2. If the negotiation code matches a stored negotiation → proceed
3. If the negotiation code doesn't match → respond with "invalid_code" error

When the negotiation code is valid:
...
3. Stores: `{complete_token, session_id, confirmation_code, negotiation_id, expires_at}`
...
**Critical:** The service MUST allow multiple claims against the same session ID (provided each has a valid negotiation code). Each claim generates a separate complete token and confirmation code. This is essential for attack detection.
...
4. The confirmation code matches the claim

Outcomes:

- **Zero claims:** Respond with "pending"--user agent hasn't claimed yet
- **One negotiation, one claim, code matches:** Mark as approved, respond with success
...
**Note on verification order:** The multi-negotiation check (step 2) and multi-claim check (step 3) can logically precede the confirmation code check (step 4)—if multiple negotiations or claims exist, the ceremony must abort regardless of whether the code matches. However, services may still choose to verify the confirmation code even in abort scenarios for logging or audit purposes.
...
## 6. The Negotiation Code

The negotiation code is a critical security feature that prevents session fixation attacks. This section explains the attack it prevents, why it's necessary, and how it works.
...
6. User's browser claims, displays confirmation code (attacker observes it)
7. Attacker enters the confirmation code into their app, approves
...
- **Confirmation code** doesn't help—the attacker observes it and enters it correctly
- **The attacker needs no access to the user's devices**—only visual observation

### 6.2 The Solution: Negotiation Code

The negotiation code creates symmetry: just as the confirmation code binds the user's browser to the approval, the negotiation code binds the user's app to the claim.

**The flow with negotiation code:**

1. User's browser displays QR code **and a negotiation code input field**
2. User's app scans QR code, negotiates with service
3. Service returns `negotiation_code` to the app (along with `approve_token`)
4. User reads negotiation code from app, enters it into browser
5. Browser sends claim request **including the negotiation code**
6. Service validates: does the negotiation code match? Are there multiple negotiations?
7. If valid, browser receives `complete_token` and `confirmation_code`
8. User enters confirmation code into app, app approves
9. Browser completes, receives result

**Why this defeats the attack:**

If the attacker negotiates first:
- Attacker's app receives negotiation code "ABC"
- User's app also negotiates, receives different negotiation code "XYZ"
- Multi-negotiation detection triggers—ceremony aborts

If the attacker negotiates and user hasn't scanned yet:
- Attacker's app receives negotiation code "ABC"
- User's browser is waiting for negotiation code input
...
**The critical insight:** An attacker who can only *observe* the user's screen cannot enter the negotiation code. They'd need to type on the user's keyboard. If they can do that, they can directly manipulate any authentication mechanism.
...
3. If exactly one → proceed with confirmation code check
...
- But then attacker must enter negotiation code into user's browser (impossible without keyboard access)

### 6.4 Negotiation Code Specification

The service controls the negotiation code format. There are two options for how the web page learns the format:
...
navigator.outOfBandBinding.request({
  // ... other fields ...
  negotiationCodeSpecEndpoint: "/bind/negotiation-spec"
});
```
...
- Character sets follow the same rules as confirmation codes (Section 7.3)
- Length: 1-6 characters (same bounds as confirmation codes)
...
- User's app negotiates → success, receives negotiation code
...
- Attacker's app negotiates → success, receives negotiation code
...
- User's browser waits for negotiation code input
...
If an attacker can enter the negotiation code into the user's browser (physical keyboard access, remote desktop, malware with input injection), then the negotiation code provides no protection. The attacker can:

1. Observe QR code
2. Negotiate as attacker
3. Enter negotiation code into user's browser
4. Wait for user's browser to display confirmation code
5. Enter confirmation code into their app
...
The negotiation code protects against *observation-only* attackers (shoulder-surfing, screen capture, camera surveillance). Attackers with input access require physical security measures, not protocol defenses.

---

## 7. The Confirmation Code
...
Different services have different security requirements. A forum might be satisfied with a less-secure login than a bank. This proposal allows services to control the confirmation code specification within defined bounds.
...
The user agent (browser) displays the confirmation code to the user. Browsers have comprehensive Unicode support, so display capability is not a practical constraint. The browser does not need advance knowledge of the character set--it simply renders whatever code the service provides.
...
The confirmation code and multi-claim detection work together as an integrated security mechanism. Neither is sufficient alone; both are essential.
...
If an attacker can replace the browser's trusted UI with a fake one displaying the attacker's confirmation code, the user might enter the attacker's code. However, this attack requires the attacker to have control over what the user sees on their own screen—a level of compromise (screen injection, browser takeover) where the attacker has already won regardless of this protocol. No protocol can protect a user whose display is controlled by an attacker.

**The confirmation code also provides psychological assurance:** seeing the same code on browser and app confirms the user is participating in a single, consistent ceremony.
...
Even if the attacker claims first and the user enters a code prematurely, the probability of matching the attacker's code is low (see Section 7).
...
The negotiation code prevents this attack. When the companion app negotiates, it receives a negotiation code. The user must enter this code into the browser before claiming can occur. An attacker who only observes the QR code cannot enter the negotiation code—they'd need keyboard access to the user's browser.
...
**Wrong code handling.** When the user enters an incorrect confirmation code, the companion application SHOULD allow retry. Wrong code attempts do not affect ceremony state—the user can keep trying until the session expires. This is safe because multi-claim detection, not the confirmation code, provides the security guarantee (see Section 7.5).

Unlimited retries are not a security risk as long as the approve token remains secret between the service and companion application. An attacker who has only observed the QR code cannot attempt confirmations—they lack the approve token. The confirmation code mechanism prevents only the specific race condition described in Section 7.5, which unlimited retries do not affect.
...
### 11.4 Browser-Server Communication
...
- **No trusted UI.** The browser renders the QR code, displays the origin, shows the confirmation code. You provide display text; the browser handles presentation.
...
- Which apps have negotiated (each with an approve_token and negotiation_code)
- The result data to deliver upon success (per negotiation)
- Which browsers have claimed (each with a complete_token and confirmation_code, linked to a negotiation)
...
|----------|----------|------|---------|
| initiate | session_id, operation data | Validate operation, store result + approve_token + negotiation_code | approve_token, negotiation_code, code_spec |
| claim | session_id, negotiation_code | Validate negotiation_code matches, record claim with new complete_token + confirmation_code | complete_token, confirmation_code |
| approve | approve_token, code | Count negotiations, count claims, verify code matches, record approval | status |
...
### Q: Why are there TWO user-entered codes (negotiation and confirmation)?

They protect against different attacks with symmetric two-phase verification:

- **Negotiation code (app → browser):** Prevents session fixation. An attacker who observes the QR code could negotiate as themselves before the user scans. Without the negotiation code, the user's browser would end up with the attacker's session. The negotiation code requires the user to enter something from *their own* app, which an observation-only attacker cannot do.

- **Confirmation code (browser → app):** Prevents session hijacking. An attacker who observes the QR code could claim before the user's browser does. Without the confirmation code, the attacker could approve and steal the result. The confirmation code requires the user to enter something from *their own* browser, which an observation-only attacker cannot do.
...
Partially. A JavaScript library could implement most of the flow: generate session IDs, render QR codes, poll endpoints, display confirmation codes. However, a polyfill cannot provide the key security property: a trusted UI that the web page cannot manipulate. In a polyfill, the "trusted" UI is just more JavaScript that a malicious page could spoof or interfere with.
...
The `supported_charsets` field declares which confirmation code character sets the companion application can handle. All compliant apps MUST include at least `"0-9"`. The `operation_data` field is service-specific (e.g., authentication assertion, payment details).
...
The `negotiation_code` is displayed to the user by the companion app. The user enters it into the browser's trusted UI before the browser claims.
...
Called by the browser to register interest in receiving the result. The browser includes the negotiation code entered by the user.

**Request:**
```json
GET {claimEndpoint}?session_id=a]'k2...&negotiation_code=K7M2
...
**Response (invalid negotiation code):**
```json
HTTP/1.1 200 OK
Content-Type: application/json

{
  "status": "error",
  "reason": "invalid_code",
  "message": "Negotiation code does not match any negotiation"
}
```

**Response (ready - valid negotiation code):**
```json
HTTP/1.1 200 OK
Content-Type: application/json
...
```
event: claim_ready
data: {"complete_token":"j8Lp3...","confirmation_code":"47"}
