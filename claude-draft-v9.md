...
**Version:** 0.9 (Draft for public comment)
...
**Wrong code handling.** When the user enters an incorrect confirmation code, the service responds with `{status: "error", reason: "invalid_code"}`. The companion application SHOULD allow the user to retry.

Services MAY implement retry limits to mitigate brute-force attempts, though the combination of short expiration times (60-120 seconds) and small code spaces (maximum 6 characters) means brute-force is already impractical. After exhausting retries, the service SHOULD respond with `{status: "error", reason: "retry_limit_exceeded"}`.

The ceremony state is not affected by wrong code attempts—the user can keep trying until the session expires or they enter the correct code. This is safe because the confirmation code is not the security mechanism; multi-claim detection is (see Section 6.5).
...
**Polling (default).** Browser periodically calls `claimEndpoint` and `completeEndpoint` via HTTP. Simple, stateless, works through all proxies and firewalls. For ceremonies under two minutes, polling at 1-2 second intervals adds negligible load.

**WebSocket.** Browser establishes a WebSocket connection to `statusEndpoint`. Server pushes status updates when claim is ready and when approval is complete. More efficient for services already using WebSocket infrastructure.

**Server-Sent Events (SSE).** Browser opens an SSE connection to `statusEndpoint`. Server pushes status updates. Simpler than WebSocket (HTTP-based, unidirectional) while avoiding polling overhead.

The protocol semantics (what data is exchanged, what checks are performed) remain identical regardless of transport. The service picks the mode that matches its infrastructure; if frontend and backend don't match (e.g., JS specifies WebSocket but server doesn't support it), the ceremony fails—that's the service's responsibility.
...
Estimated effort: Minutes, once endpoints exist.
...
Estimated effort: A few weeks of engineering for a browser team familiar with the codebase. Comparable to implementing other web platform APIs.
...
Estimated effort: A few days for a mobile developer. The app is simpler than the browser—it makes two HTTP requests and displays three screens.
...
Estimated effort: A day or two for a backend developer. Reference implementations could reduce this to hours of configuration.
